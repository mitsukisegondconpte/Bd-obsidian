...
import { BookOpen, ExternalLink, Hash, User, Users } from 'lucide-react'
...
// exclut 'work' puisque c'est déjà le contenu natif de cette app.
export default function CrossPlatformResults({ query }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      globalSearch(q)
        .then((data) => setResults(data.filter((r) => r.result_type !== 'work')))
        .catch(() => setResults([]))
    }, 300)
