...
export async function createComment({ userId, chapterId, body, parentCommentId }) {
  const { data, error } = await supabase
    .from('comments')
    .insert({ user_id: userId, target_type: 'chapter', target_id: chapterId, body, parent_comment_id: parentCommentId ?? null })
    .select('*, user:profiles(username, display_name, avatar_url)')
    .single()
  if (error) throw error
